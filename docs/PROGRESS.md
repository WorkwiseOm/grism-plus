...
- CI uses `npm install --no-audit --no-fund` instead of `npm ci` for cross-platform lockfile tolerance. Reason: `@rolldown/binding-wasm32-wasi@1.0.0-rc.17` (transitive dep of `vitest@4`) declares exact-version deps `@emnapi/core@1.10.0` and `@emnapi/runtime@1.10.0`, but npm 11 on Windows skips writing resolution entries for them because the binding has `cpu: wasm32` (which never matches a real install platform). CI's `npm ci` then rejects the lockfile as "Missing... from lock file" even though the runtime tree is consistent for the actual platform. `npm install` in CI is lenient about lockfile inconsistencies and updates them in-place. Trade-off: CI loses the strict reproducibility guarantee of `npm ci` (i.e., "exactly the lockfile, no surprises"). Version pinning via `package-lock.json` is preserved; the lookup is just permissive about platform-skewed resolution gaps. Phase 4 revisit: switch back to `npm ci` if reproducibility friction surfaces (e.g., flaky CI on dep-bump PRs, or audit/compliance review wants stricter install discipline) or if a Linux-only build matrix becomes practical (matrix CI on a Linux runner can regenerate the lockfile in a way that satisfies the validator).
